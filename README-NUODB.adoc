...
. Checkout branch nuodb/7.0 or nuodb/7.2 as needed.

. Windows users are recommended to use `git bash` or WSL rather than `CMD` or _PowerShell_.

. You must have Java JDK 17 or 21 installed.  Java 8 and 11 aren't supported by Hibernate 7.x, it requires Java 17 or later.
...
** There is a `.java-version` file in this project, telling `jenv` to use Java 17.
** If necessary, use `jenv` to switch to Java 17.
For example, to use 17.0.14, run `jenv local 17.0.14`.
* _Option 2:_ use `sdkman`: https://sdkman.io
** `skdman` requires `git bash` or WSL on Windows.
** To set the same java version: `sdk use java 17.0.14`

. Next, make sure you have our Hibernate 7 dialect jar available:
...
* Check the version in the POM - it will be of the form `nuodb-hibernate.NN.x.x-hib7`
   * You will need to set `DIALECT_VERSION` to `NN.x.x-hib7` to match - see below.
...
* **Note:** the value you set _should_ have `-hib7` in the end, but that's optional:
+
[source%autofit,sh,subs="verbatim,attributes"]
----
...
Tests can take 40-60 minutes to run depending on your machine.
...
** _15404 tests completed, 0 failed, 2606 skipped_ (NuoDB V6.02)
